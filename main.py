...
def sendmail(msg):
    sender = 'abmail07@gmail.com'
    password = 'otanncvowjiisyly'
    sendto = 'abmail07@gmail.com'
    with smtplib.SMTP('smtp.gmail.com',587)as smtp:
        smtp.ehlo()
        smtp.starttls()
        smtp.ehlo()
        smtp.login(sender, password)
        smtp.sendmail(sender, sendto, msg)
...
def refresh():
    ...
    for activity in raw_data:
        if activity['sport_type'] != 'Run' and int(activity['distance']) < 100:
            ...
            elv_gain = (activity["total_elevation_gain"])
            try:
                pace =  60/(activity_speed_m*3.6)
            except:
                pace = 0
            sql = 'INSERT INTO all_activities (date, distance, pace, avg_hr, max_hr, gear, latitude, longitude, elv_gain) VALUES(?,?,?,?,?,?,?,?,?)'
            val = (date, distance_meter, pace, activity_heart, max_heartrate, gear_id, latitude, longitude, elv_gain)
            with connection:
                cursor.execute(sql,val)
    print(x)
...
def add_location_th01():
    ...
    for key,value in run_loc01:
        ...
        for answer in value:
            ...
            connection = sqlite3.connect('answers.db')
            cursor = connection.cursor()
            val = (answer[0], answer[1], answer[2], answer[3], answer[4], answer[5], neighbourhood, city, state, elv_gain)
            sql = """INSERT INTO %s (date, distance, pace, avg_hr, max_hr, gear, neighbourhood, city, state, elv_gain) VALUES (?,?,?,?,?,?,?,?,?,?)""" % key
            with connection:
                cursor.execute(sql, val)
...
def print_last_run():
    ...
    if shoe_ks[0][0] > 400:
        subject = "Shoe Miles Allert"
        body = f'you ran {round(shoe_ks[0][0]),1} in {shoe[0]}, Time to look for a new pair :)'
        msg01 = f'Subject: {subject}\n\n{body}'.encode('utf-8')
        sendmail(msg01)
...
def mile_prediction():
    ...
    for item in all:
        ...
        year = int(date[0:4])
        if year == (int(today)-1):
            last_year = (last_year + item[1])
        else:
            continue


    text_box = Text(root, height=text_box_hight, width=50, bg = text_box_bg, font=(fnt,fnt_size))
    to_print = (f'{round(mile_for_now/1000,2)}Km this year so far\nAll year miles prediction: {int(year_prediction)}Km\nLast Year: {int(last_year/1000)}Km\n\n')
...
def load_frame():
    ...
    bt06 = Button(root,text="Empty", font=(fnt,fnt_size), fg=fg, bg=button_bg, activebackground = abg,
    activeforeground = afg, height = hi, width = wid)
